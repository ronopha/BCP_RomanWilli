...
//++++++++++++++++++++++++++++++++++++++++++++++++++Start of the Account Management module++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    */
        ...
        returns (int64)
    {
        ...
        emit newSender(SenderID, _PIN, _PUK, _PUK2a, _PUK2b, _PUK2c, _gasPrice);
        return(SenderID);
    }
    ...
    */
    ...
    /**
    @dev to change the _gasPrice of a Sender
    @dev has to be called by the PUK
    @param _newGasPrice is the new __gasPrice for the Sender
    */
    function ChangeGasPrice(uint64 _newGasPrice) external {
        // 1. Authentication
        int64 senderID = keyMap[msg.sender];
        require(msg.sender == senders[uint(senderID)]._PUK, "Not authorized!");
        Sender storage s = senders[uint (senderID)];
        s._gasPrice = _newGasPrice;
        emit GasPriceChanged(senderID, _newGasPrice);
    }
    ...
    // 4. Case distinction
        if (s._PUK == s._PUK2a && 
            ...
            s._PUK2b = _newPUK2b;
            s._PUK2c = _newPUK2c;
            resetIndex[SenderID] = ResetPUK(address(0), Sender(address(0),address(0),address(0),address(0),address(0),0));
            emit resetPINPUK(SenderID, _newPIN, _newPUK, _newPUK2a, _newPUK2b, _newPUK2c);
            return true;
        } else {
            resetIndex[SenderID] = ResetPUK(address(0), Sender(address(0),address(0),address(0),address(0),address(0),0));
...
//++++++++++++++++++++++++++++++++++++++++++++++++++Start of the Commitment module+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   function NewCommitment(
    ...
    function HorizonExtension(int64 commitmentID, uint32 _newHorizon) external {
        ...
        require(msg.sender == senders[uint(c.SenderID)]._PUK, "Not authorized!");
...
//+++++++++++++++++++++++++++++++++++++++++++++++Start of the Order & Delivery module++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    */
    ...
    function GetTransactionCosts(int64 _commitmentID, uint40 _gasForMailbox) external view returns (uint) {
        ...
        return uint((_gasForMailbox+_gasForRelay) * _gasprice + _senderFee);
    }
    ...
    // 1. Order analysis
        ...
        uint _gasCost = _gasForDelivery * senders[uint(c.SenderID)]._gasPrice;
...
//+++++++++++++++++++++++++++++++++++++++++++++++Start of the Contract Information Module++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    function GetSenderInformation(int64 SenderID) public view returns (address, address, address, address, address) {
        ...
        /**
    @dev returns the defined Gasprice of a sender
    @dev has to be called by the PUK
    @param SenderID uint that identifies a specific sender (is constant)
     */
    function GetGasPrice(int64 SenderID) public view returns (uint64) {
        require (SenderID >= 0, "Must be a SenderID >= 0!"); 
...
//+++++++++++++++++++++++++++++++++++++++++++++++Start of the Contract Governance module++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    */
    ...
    constructor(address payable _PIN, address payable _PUK, address _PUK2a, address _PUK2b, address _PUK2c) {
        ...
        senders.push(Sender(_PIN, _PUK, _PUK2a, _PUK2b, _PUK2c, 12 gwei));
        keyMap[_PIN] = 0;
        keyMap[_PUK] = 0;
